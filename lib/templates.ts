// lib/templates.ts

export const templates = [
  {
    category: "BATTERY",
    items: [
      "BATTERY: Drains Too Fast/Not Holding Charge",
      "BATTERY: Drains Too Fast/Not Holding Charge - Zingtree Follow-Up",
      "BATTERY: Not Charging",
      "BATTERY: Not Turning Off",
      "BATTERY: Not Turning On",
      "BATTERY: Only Works When Plugged In",
      "BATTERY: TWS Charging Issues",
    ],
  },
  {
    category: "SOUND (H)",
    items: [
      "SOUND (H): Buzzing/Distortion/Popping/Static",
      "SOUND (H): Dropping Out / Intermittent Sound (bluetooth)",
      "SOUND (H): Dropping Out / Intermittent Sound (wired)",
      "SOUND (H): No Sound In Either Ear (in-ear TWS)",
      "SOUND (H): No Sound In Either Ear (in-ear wired)",
      "SOUND (H): No Sound In Either Ear (in-ear wireless Non-TWS)",
      "SOUND (H): No Sound In Either Ear (other on-ear, etc. - wired)",
      "SOUND (H): No Sound In Either Ear (other on-ear, etc. - wireless)",
      "SOUND (H): No Sound In L or R Ear (in-ear wired)",
      "SOUND (H): No Sound In L or R Ear (other on-ear, etc. - wired)",
      "SOUND (H): No Sound In L or R Ear (other on-ear, etc. - wireless)",
      "SOUND (H): No Sound In L or R Ear (TWS ONLY)",
      "SOUND (H): Not Enough Bass / Tinny (earbuds)",
      "SOUND (H): Not Enough Bass / Tinny (in-ears)",
      "SOUND (H): Not Enough Bass / Tinny (in-ears) (Check My Best Fit & Personi-Fi)",
      "SOUND (H): Not Enough Bass / Tinny (in-ears) (Check My Best Fit)",
      "SOUND (H): Volume Low/Imbalanced (in-ears only)",
      "SOUND (H): Volume Low/Imbalanced (in-ears only)(Check My Best Fit & Personi-Fi)",
      "SOUND (H): Volume Low/Imbalanced (in-ears only)(Check My Best Fit)",
    ],
  },
  {
    category: "SOUND (P)",
    items: [
      "SOUND (P): Buzzing / Distortion / Popping / Static",
      "SOUND (P): Dropping out / Intermittent Sound (wired)",
      "SOUND (P): Dropping out / Intermittent Sound using (bluetooth)",
      "SOUND (P): Interference",
      "SOUND (P): Low Volume - Bluetooth",
      "SOUND (P): Low Volume - Wired",
      "SOUND (P): Muffled",
      "SOUND (P): No Sound (Wired)",
      "SOUND (P): No Sound (Wireless)",
      "SOUND (P): Not Enough Bass/Tinny",
      "SOUND (P): Too Much Bass",
      "SOUND (P): Volume Suddenly Changes On Its Own",
    ],
  },
  {
    category: "Bluetooth",
    items: [
      "BLUETOOTH: Asking For Pin (product does not support voice assistance)",
      "BLUETOOTH: Asking For Pin (product supports voice assistance)",
      "BLUETOOTH: Connecting/Active Issue",
      "BLUETOOTH: Left and Right ear pieces not pairing together",
      "BLUETOOTH: Pairing Issue",
    ]
  }
];